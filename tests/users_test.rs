...
mod test {
    ...
    async fn test_create_existing_user() {
        ...
        client.with_admin("create_existing_user@mail.com", &mut db).await;
        let user = doc!{
            "username":"Matias",
            "email": "create_existing_user@mail.com",
            "password": "some_pass"
    ...
    async fn test_create_user(){
        let mut db = DbFixture::new().await;
        let mut client = LoggedClient::init().await;
        client.with_admin("admin@mail.com", &mut db).await;
    ...
    async fn test_get_users() {
        let mut client = LoggedClient::init().await;
        let mut db = DbFixture::new().await;
        client.with_admin("test_get_users@email.com", &mut db).await;
